...
void modeBusInit()
{
	...
	init_timer32(0, ((SystemCoreClock/LPC_SYSCON->SYSAHBCLKDIV)/((BAUDDRATE/15))),1); //10+3.5+1.5 bit (modbus say 3.5 betwen 2 paquet but need ad time of 1byte 10bit and +1.5 for security)
...
void modeBusUartRx(uint8_t c)
{
	...
	switch(state)
	{
		...
		case Smb_address:
			...
			{
				if((paquet.address==config.modeBusaddress) || (paquet.address==0))
					state=Smb_FUNCTION;
				else
					state=Smb_WAIT;
			}
	...
	switch (paquet.function)
	{
		...
		case F_WRITE_REGS:
			...
			/*if(paquet.function==F_WRITE_REGS)
				nbByteByVal=nbByte/nbValue;*/ //every time 2
			posValue++;
		case F_WRITE_REG:
		case F_WRITE_COIL:
			val_bool=0;
