 ...
 *   Some parts Code Red Technology (c) 2009,2010
 *   All rights reserved.
 *
 *   History
 *   2008.08.20  ver 1.00    Preliminary version, first Release
 *   2009            1.01    CodeRed updates to extract out interrupt handler code
 *
******************************************************************************/
#include "LPC11xx.h"
#include "timer32.h"
...
** 
******************************************************************************/
void init_timer32(uint8_t timer_num, uint32_t TimerInterval,uint8_t enable_irq)
{
  if ( timer_num == 0 )
  {
...
//    LPC_IOCON->PIO1_4 |= 0x02;		/* Timer0_32 MAT3 */
    ...
    LPC_TMR32B1->PWMC = 0x3;// match1 and 2 enable
    LPC_TMR32B1->MR3 = TimerInterval;
//	LPC_TMR32B1->EMR &= ~(0xFF<<4);
//	LPC_TMR32B1->EMR |= (0x03<<10);	/* MR3 Toggle */
    LPC_TMR32B1->MCR = (3<<9)|(1<<6);			/* Interrupt and Reset on MR3 Interrupt MR2 */
