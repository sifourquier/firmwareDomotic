...
void firmwareStore(int adresse,uint8_t* buf, int size)//size need a value = 256*2^x
{
	...
	{
		iapPrepareSector(adresse/SECTORSIZE,adresse/SECTORSIZE);

		iapEraseSector(adresse/SECTORSIZE, adresse/SECTORSIZE);
	}
	iapPrepareSector(adresse/SECTORSIZE,adresse/SECTORSIZE);
	iapCopyRamToFlash((void*)buf,(void*)adresse,size);
	__enable_irq();
}
