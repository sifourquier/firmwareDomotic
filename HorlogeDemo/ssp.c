 ...
 ** parameters:			None
 ** Returned value:		None
 ** cphacpoll = 0b000000ab a=cpha b=cpoll
 *****************************************************************************/
void SSP_Init( uint8_t portNum ,uint32_t frequ ,uint8_t nbBit,uint8_t cphacpoll)
{
	...
	if ( portNum == 0 )
	{
		...
		LPC_SSP0->CR0 = (cphacpoll<<6/*CPOL*/)|(0<<8/*div by 2 watch CPSR*/ )|(nbBit-1);

		/* SSPCPSR clock prescale register, master mode, minimum divisor is 0x02 */
		;
		LPC_SSP0->CPSR = SystemCoreClock/2/frequ/2*2; //div 4mHz 48/2/6 doit etre paire;
		//pour lcd 150k
	...
	{
		/* Set DSS data to 8-bit, Frame format SPI, CPOL = 0, CPHA = 0, and SCR is 15 */
		LPC_SSP1->CR0 = (cphacpoll<<6/*CPOL*/)|(0<<8/*div by 2 watch CPSR*/ )|(nbBit-1);
...
void SSP_Send_not_bloking( uint8_t portNum, uint8_t data)
{
	uint8_t Dummy = Dummy;
...
uint8_t SSP_Ready_for_send( uint8_t portNum)
{
	uint8_t Dummy = Dummy;
	uint8_t ret;
	...
	if ( portNum == 0 )
	{
		ret= ( (LPC_SSP0->SR & (SSPSR_BSY)) != SSPSR_BSY );
		/* Whenever a byte is written, MISO FIFO counter increments, Clear FIFO
	on MISO. Otherwise, when SSP0Receive() is called, previous data byte
	is left in the FIFO. */
		if(ret)
	...
	{
		ret= ( (LPC_SSP1->SR & (SSPSR_BSY)) != SSPSR_BSY );
		/* Whenever a byte is written, MISO FIFO counter increments, Clear FIFO
	  on MISO. Otherwise, when SSP0Receive() is called, previous data byte
	  is left in the FIFO. */
		if(ret)
...
void SSP_Send_16bit_nowait_end( uint8_t portNum, uint16_t data)
{
	uint8_t Dummy = Dummy;
...
void SSP_wait_end( uint8_t portNum)
{
	uint8_t Dummy = Dummy;
